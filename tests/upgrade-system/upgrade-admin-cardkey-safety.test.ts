...
databaseTest('manualRequired 失败任务不能取消，且本站卡密继续保持占用', async () => {
...
databaseTest('人工标记成功时，会把选中的渠道卡密标记为已使用并回填任务', async () => {
...
databaseTest('成功任务可更换手工绑定错的渠道卡密，并释放旧的手工绑定卡密', async () => {
...
databaseTest('批量禁用渠道卡密只处理可用卡密，不改动已使用和锁定卡密', async () => {
...
databaseTest('批量启用渠道卡密只恢复已禁用卡密，不改动可用、已使用和锁定卡密', async () => {
...
databaseTest('同一渠道下重复插入相同卡密会被数据库唯一约束拒绝', async () => {

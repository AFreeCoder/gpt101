...
databaseTest('pickAndRunTasks 遇到 9977 型终止失败时不再尝试下一个渠道，且保留本站卡密与渠道卡密占用', async () => {
...
databaseTest('pickAndRunTasks 对普通失败仍会释放卡密并切换到下一个渠道', async () => {
...
databaseTest('pickAndRunTasks 遇到 9977 无充值记录失败时释放本站卡密和渠道卡密', async () => {
  ...
  try {
    ...
    const [redeemRow] = await db()
      ...
      .from(redeemCode)
      .where(eq(redeemCode.id, seeded.redeemCodeId));
    assert.equal(redeemRow.status, 'available');
    assert.equal(redeemRow.usedByTaskId, null);
    ...
    const [retriedTaskRow] = await db()
      .select()
      .from(upgradeTask)
      .where(eq(upgradeTask.id, seeded.taskId));
    const [retriedRedeemRow] = await db()
      .select()
      .from(redeemCode)
      .where(eq(redeemCode.id, seeded.redeemCodeId));
    assert.equal(retriedTaskRow.status, UpgradeTaskStatus.PENDING);
    assert.equal(retriedRedeemRow.status, 'consumed');
    assert.equal(retriedRedeemRow.usedByTaskId, seeded.taskId);
  } finally {
    await cleanupByPrefix(prefix);
  }
});

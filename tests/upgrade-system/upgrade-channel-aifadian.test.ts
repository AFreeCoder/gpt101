...
test('aifadian 充值异常后二次验卡已用且匹配当前邮箱和时间时判成功', async () => {
  const calls: Array<{ url: string; body: any }> = [];
  ...
  const adapter = createAifadianAdapter({
    ...
    fetchImpl: (async (input, init) => {
      ...
      if (url.endsWith('/verify/cdk')) {
        ...
        assert.ok(response, 'verify 响应已耗尽');
  ...
  const result = await adapter.execute({
    taskId: 'task_aifadian_confirm_used',
    productCode: 'plus',
    memberType: 'month',
    sessionToken: buildSessionJson(),
    chatgptEmail: 'user@example.com',
    channelCardkey: 'AIFADIAN-CONFIRM-USED',
  });
  ...
  assert.deepEqual(
    ...
    ['/api/verify/cdk', '/api/recharge', '/api/verify/cdk']
  );
});

test('aifadian 二次验卡已用但邮箱不匹配时仍转人工处理', async () => {
  const verifyResponses = [
    { status: 'valid' },
    {
      status: 'used',
  ...
  const result = await adapter.execute({
    taskId: 'task_aifadian_other_email',
    productCode: 'plus',
    memberType: 'month',
    sessionToken: buildSessionJson(),
    chatgptEmail: 'user@example.com',
    channelCardkey: 'AIFADIAN-OTHER-EMAIL',
  });

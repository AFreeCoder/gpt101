...
databaseTest('authenticatePartnerRequest 校验 HMAC 签名并拒绝重复 nonce', async () => {
...
databaseTest('authenticatePartnerRequest 清理过期 nonce 并记录失败审计', async () => {
...
databaseTest('authenticatePartnerRequest 限制失败鉴权审计写入量', async () => {
...
databaseTest('partner 鉴权默认不信任 forwarded 头，除非显式开启可信代理头', async () => {
...
databaseTest('createPartnerApp 分发独立密钥且 rotatePartnerAppSecret 使旧密钥失效', async () => {
...
databaseTest('partner app 支持查询和软删除且名称不可修改', async () => {
...
databaseTest('verifyPartnerOrder 拒绝未显式配置的 partner 商品', async () => {
...
databaseTest('verifyPartnerOrder 幂等创建外部订单到内部升级凭证的映射', async () => {
...
databaseTest('submitPartnerUpgradeTask 复用同一外部订单任务并限制 taskNo 归属', async () => {
...
databaseTest('submitPartnerUpgradeTask 并发提交同一外部订单时返回同一个 taskNo', async () => {
...
databaseTest('resolvePartnerAccount 不向第三方返回 accessToken', async () => {
...
databaseTest('partner verify-order route 要求签名并返回内部订单映射结果', async () => {

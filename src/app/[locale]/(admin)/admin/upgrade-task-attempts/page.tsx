...
export default function UpgradeTaskAttemptsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/upgrade-task-attempts/list?${params}`);
      const data = await res.json();
      if (data.code === 0) { setAttempts(data.data.items); setTotal(data.data.total); }
    } catch {}
    setLoading(false);
    ...
    <div className="p-6">
      ...
      <div className="mb-4">
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          placeholder="搜索任务编号" className="rounded-lg border px-3 py-1.5 text-sm w-80" />
      </div>
            ...
            {loading ? (
              ...
              attempts.map((a) => (
                <tr key={a.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-3 py-2 text-xs text-gray-500 max-w-64 truncate" title={a.errorMessage || ''}>
                    {a.errorMessage || '-'}
                  </td>
                  <td className="px-3 py-2 text-xs text-gray-500">
                    {a.durationMs != null ? `${(a.durationMs / 1000).toFixed(1)}s` : '-'}
                  </td>
                  <td className="px-3 py-2 text-xs text-gray-500">{new Date(a.startedAt).toLocaleString('zh-CN')}</td>
                  <td className="px-3 py-2 text-xs text-gray-500">{a.finishedAt ? new Date(a.finishedAt).toLocaleString('zh-CN') : '-'}</td>
                </tr>
              ))
            )}
          </tbody>

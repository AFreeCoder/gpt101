...
export default function ChannelCardkeysPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/upgrade-channels/list')
      .then((r) => r.json())
      .then((d) => {
        if (d.code === 0) setChannels(d.data);
      });
  }, []);
  ...
  const handleBatchDelete = async () => {
    // 过滤掉 used 状态的
    const deletable = cardkeys.filter(
      (c) =>
        selectedIds.has(c.id) && c.status !== 'used' && c.status !== 'locked'
    );
    if (deletable.length === 0) {
      alert('选中的卡密中没有可删除的（已使用/锁定的不能删除）');
      return;
    }

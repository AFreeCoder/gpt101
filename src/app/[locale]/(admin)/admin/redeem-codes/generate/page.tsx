...
export default function GenerateRedeemCodesPage() {
  ...
  const [prefix, setPrefix] = useState('GPT101');
  ...
  const handleGenerate = async () => {
    ...
    if (count < 1 || count > 5000) {
      setError('数量范围 1-5000');
      return;
    }
    if (!validateRedeemCodePrefix(prefix)) {
      setError('卡密前缀只能包含 2-20 位字母或数字');
      return;
    }
    ...
    try {
      const res = await fetch('/api/admin/redeem-codes/generate', {
        ...
        body: JSON.stringify({
          ...
          count,
          unitPrice,
          prefix: normalizeRedeemCodePrefix(prefix),
        }),
      });
  ...
  return (
    <>
      ...
      <div className="p-6">
        ...
        {error && (
          <div className="mb-4 max-w-xl rounded-lg bg-red-50 p-3 text-sm text-red-600">
            {error}
          </div>
        )}

        <div className="bg-card max-w-xl space-y-4 rounded-xl border p-6">
          <div>
          ...
          <div>
            <label className="mb-1 block text-sm font-medium">卡密前缀</label>
            <input
              type="text"
              value={prefix}
            ...
            <p className="text-muted-foreground mt-1 text-xs">
              2-20 位字母或数字；例如 GPT101、KOL88、MEDIA01。
            </p>
          </div>
              ...
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    批次: {resultTitle}，共 {resultCodes.length} 张
                  </p>
                </div>
                <button
                  onClick={() => setShowResult(false)}

...
export function getAnalyticsManagerWithConfigs(configs: Configs) {
  ...
  // google ads
  if (configs.google_ads_id) {
    analytics.addProvider(
      new GoogleAdsProvider({ adsId: configs.google_ads_id })
    );
  }

  // 百度统计
  if (configs.baidu_analytics_id) {
    analytics.addProvider(
      new BaiduAnalyticsProvider({ baiduId: configs.baidu_analytics_id })
    );
  }
